# This Makefile is for the Memoize::ExpireLRU extension to perl.
#
...
#     NAME => q[Memoize::ExpireLRU]
#     PREREQ_PM => { Memoize=>q[0.52] }
#     VERSION_FROM => q[ExpireLRU.pm]
...
MAKE = make
FULLEXT = Memoize/ExpireLRU
BASEEXT = ExpireLRU
PARENT_NAME = Memoize
DLBASE = $(BASEEXT)
VERSION_FROM = ExpireLRU.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = ExpireLRU.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Memoize
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Memoize
...
TO_INST_PM = ExpireLRU.pm

PM_TO_BLIB = ExpireLRU.pm \
	$(INST_LIB)/Memoize/ExpireLRU.pm
...
DISTNAME = Memoize-ExpireLRU
DISTVNAME = Memoize-ExpireLRU-0.55
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  ExpireLRU.pm $(INST_MAN3DIR)/Memoize::ExpireLRU.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               Memoize-ExpireLRU' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    Memoize:  0.52' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,55,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Memoize" VERSION="0,52,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  ExpireLRU.pm $(INST_LIB)/Memoize/ExpireLRU.pm 
